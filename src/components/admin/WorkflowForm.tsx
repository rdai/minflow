...
export default function WorkflowForm({ workflow }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (isEdit) {
      ...
      if (error) { setError(error.message); setLoading(false); return }
      router.push(`/admin/workflows/${data.id}/edit`)
    }
    setLoading(false)
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white border border-stone-200 rounded-xl p-6 space-y-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div>
          <label className="block text-sm font-medium text-stone-700 mb-1">Goal (ministry objective)</label>
          <input
            value={category}
            onChange={(e) => setCategory(e.target.value)}
            className="w-full border border-stone-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
            placeholder="Scripture Access, Evangelism..."
        ...
        <div>
          <label className="block text-sm font-medium text-stone-700 mb-1">Medium (output type)</label>
          <select
            value={medium}
